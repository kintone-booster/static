								...
								((app) => {
									...
									.append(
										...
										.append(kb.field.activate(((res) => {
											res.elm('select').empty().assignOption(kb.config[PLUGIN_ID].ui.options.fields({placed:viewInfos.custom},(result,current) => {
												result.push({code:current.id,label:current.name});
												return result;
											}),'label','code');
											return res;
