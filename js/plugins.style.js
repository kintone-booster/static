						...
						.then((auth) => {
							if (auth)
							{
								...
								if (['create','edit'].includes(vars.type))
								{
									...
									if (setting.latest)
										if (setting.latest==JSON.stringify(latest))
										{
											finish();
											return;
										}
									performed=Object.values(latest).some((item) => (Array.isArray(item))?item.some((item) => item=='disabled'):(item=='disabled'));
									setting.latest=JSON.stringify(latest);
								}
							}
							finish();
						})
