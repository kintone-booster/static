...
((PLUGIN_ID) => {
	...
	var apply=(settings,record,workplace='record') => {
		...
		return new Promise((resolve,reject) => {
			var recurse=(index,callback) => {
				...
				var finish=() => {
					if (latest!=JSON.stringify(kb.record.simplify({fields:vars.fieldInfos.origin},record))) performed=true;
					index++;
				...
				((setting) => {
											...
											if (setting.lookupApp.value)
											{
												kb.field.load(setting.lookupApp.value).then((fieldInfos) => {
													((fields) => {
															...
															.then((records) => {
																if (records.length!=0)
																{
																	((handler) => {
																		...
																		{
																			setting.latest.lookup=simplify();
																			callback();
																		}
																	});
																}
														...
														if (targets.length!=0)
														{
															if (setting.latest.lookup)
																if (setting.latest.lookup==simplify())
																{
																	callback();
																	return;
																}
										...
										try
										{
											if (setting.userSource.value)
											{
												((handler) => {
													...
													if (targets.length!=0)
													{
														if (setting.latest.user)
															if (setting.latest.user==simplify())
															{
																callback();
																return;
															}
					...
					if (!setting.lookupApp)
					{
						kb.alert(kb.error.config.update('Boost! Action'));
						resolve({record:record,performed:false});
					}
					return setting;
				})(settings[index]));
			};
			recurse(0,() => {
				resolve({record:record,performed:performed});
			});
		});
	};
	kintone.events.on([
							...
							try
							{
								if (['create','edit','reuse'].includes(vars.type))
								{
									((settings) => {
										if (settings.length!=0)
										{
											...
											kintone.events.on(fieldInfos.changes.reduce((result,current) => {
												...
												if ((() => {
													...
													{
														res=false;
														if (latest.record==JSON.stringify(kb.record.simplify({fields:fieldInfos.origin},e.record))) latest={record:'',time:null};
														else
														{
															if (latest.time<new Date().getTime()-5000)
															{
																latest={record:'',time:null};
																res=true;
															}
														}
													}
												...
												})())
												{
													...
													.then((resp) => {
														kb.event.call('kb.style.call',{record:e.record,mobile:vars.mobile,pattern:((vars.type=='reuse')?'create':vars.type),workplace:'record'})
														.then((param) => {
															if (resp.performed || param.performed)
															{
																latest={record:JSON.stringify(kb.record.simplify({fields:fieldInfos.origin},param.record)),time:new Date().getTime()};
										...
										if (((vars.mobile)?['both','mobile']:['both','pc']).includes(current.setting.device.value) && current.setting.event.value.includes('change')) result.push(current.setting);
										return result;
									},[]));
								}
								((settings) => {
									if (settings.length!=0)
									{
										switch (vars.type)
										{
											...
											case 'process':
												((settings) => {
													if (settings.length!=0) apply(settings,e.record,'record').then((resp) => resolve(e)).catch(() => {});
													else resolve(e);
																		...
																		() => {
																			...
																			.then((records) => {
																				...
																				let deepRecurse=(index,callback) => {
																					apply([setting],records[index],'record')
																					.then((resp) => {
					...
					kb.field.load(kb.config[PLUGIN_ID].app,true).then((fieldInfos) => {
						...
						try
						{
							((settings) => {
								if (settings.length!=0)
								{
									apply(settings,e.record,(e.workplace)?e.workplace:'record')
									.then((resp) => {
	...
	kb.event.on('kb.action.installed',(e) => {
		return {installed:vars.config.some((item) => (((e.mobile)?['both','mobile']:['both','pc']).includes(item.setting.device.value) && item.setting.event.value.includes('change')))};
	});
})(kintone.$PLUGIN_ID);
