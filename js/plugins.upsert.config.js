								...
								var setup=(tab,app,record) => {
									kb.record.set(tab.panel,app,(() => {
										...
										tab.panel.elm('[field-id=app]').elm('select').val(record.app.value).rebuild().then((fields) => {
											...
											if (record.pattern.value!='insert') tab.tables.criteria.closest('section').show().previousSibling.show().nextSibling.nextSibling.show().nextSibling.show();
										});
										return record;
									})());
								};
											...
											((app) => {
												...
												kb.event.on('kb.change.pattern',(e) => {
													if (e.container==tab.panel)
													{
														...
														{
															case 'insert':
																tab.tables.criteria.closest('section').hide().previousSibling.hide().nextSibling.nextSibling.hide().nextSibling.hide();
																break;
															default:
																tab.tables.criteria.closest('section').show().previousSibling.show().nextSibling.nextSibling.show().nextSibling.show();
																break;
														}
													}
													return e;
...
kb.constants=kb.extend({
	config:{
		caption:{
			...
			criteria:{
				en:'Linking the source and destination when updating',
				ja:'更新時のレコードの関連付け',
				zh:'更新时的源与目标链接'
			},
			destination:{
			...
			filter:{
				en:'Further narrow down the target records for updating',
				ja:'さらに絞り込む',
				zh:'进一步缩小更新目标的记录'
			},
			formula:{
		...
		description:{
			...
			mapping:{
				en:'This plugin not only allows for copying and updating between records, but also supports special transfers such as "merged transfer" which copies or updates from a record to a sub-table, and "segmented transfer" which copies or updates from a sub-table to a record. When performing operations that involve updates, it\'s essential to ensure proper associations, so please be cautious.',
				ja:'本プラグインはレコード同士のコピーや更新だけではなく、レコードからサブテーブルへのコピーまたは更新を行う「結合転送」やサブテーブルからレコードへのコピーまたは更新を行う「分割転送」といった特殊な転送が可能となっておりますが、更新を伴う処理を行う場合は、関連付けが重要となりますので、ご注意ください。',
				zh:'此插件不仅支持记录之间的复制和更新，还支持从记录到子表的“合并传输”或从子表到记录的“分割传输”等特殊传输。当执行涉及更新的操作时，确保适当的关联是非常重要的，所以请注意。'
			}
		},
		message:{
			invalid:{
				...
				criteria:{
					en:'Please specify the association of records during updates.',
				...
				mapping:{
					en:'Please specify the field to copy or update.',
				...
				multiple:{
					en:'You cannot specify fields from different tables for the association of records during updates.',
					ja:'更新時のレコードの関連付けに異なるテーブル内のフィールドを指定することは出来ません。',
					zh:'您不能为更新时的记录关联指定不同表中的字段。'
				},
				unmatch:{
					en:'When performing a segmented transfer, you need to specify the association that matches the segmented transfer for the linkage during the update.',
					ja:'レコードの分割転送を行う場合は、更新時のレコードの関連付けにも分割転送に合致した関連付けを指定する必要があります。',
					zh:'在执行分段传输时，您需要为更新时的链接指定与分段传输匹配的关联。'
				},
				various:{
