...
window.Booooooost=class{
	...
	constructor(){
				...
				switch (method)
				{
						...
						this.api.auth((authcode) => {
							fetch(
								url,
								{
									...
									body:JSON.stringify(bst.extend({authcode:authcode},body))
								}
							)
							.then((response) => {
					...
					case 'GET':
						this.api.auth((authcode) => {
							fetch(
								((url) => {
									url.search=new URLSearchParams(bst.extend({authcode:authcode},body)).toString();
									return url;
								})(new URL(url)),
								{
									method:method,
		...
		this.api.auth=(success,fail) => {
			fetch(
				this.api.url('/api/auth'),
				{
					method:'GET',
					headers:{
						'X-Requested-With':'XMLHttpRequest'
		...
		this.api.url=(endpoint) => {
			return window.location.origin + window.location.pathname.replace(/\/$/, '')+'/'+endpoint.replace(/^\//g,'');
		};
	}
	/* show alert */
					...
					response.json().then((json) => {
						switch (response.status)
						{
							case 200:
								...
								this.elms('input,select,textarea').each((element,index) => element.initialize());
								/* callback */
...
if (!window.bst)
{
	window.bst=new Booooooost();
	window.bst.error=new BooooooostError();
	window.bst.event=new BooooooostEvent();
