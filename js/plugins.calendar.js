	...
	var apply=(container,setting) => {
		((viewInfo) => {
			var calendar=new FullCalendar.Calendar(container.addClass('kb-calendar'),{
				...
				eventChange:(info) => {
					kb.view.records.set(vars.app.id,{
						put:(() => {
					...
					info.el.elms('.fc-event-time,.fc-event-title').each((element,index) => {
						...
						.on('click',(e) => {
							kb.confirm(kb.constants.calendar.message.confirm.edit[kb.operator.language],() => {
								sessionStorage.setItem('kb-calendar-edit',JSON.stringify({href:window.location.href}));
				...
				events:(info,success,failure) => {
					...
					.then((resp) => {
						var recurse=(index,callback) => {
							var finish=() => {
								index++;
								if (index<resp.length) recurse(index,callback);
								else callback();
							};
							kb.event.call('kb.style.call',{record:resp[index],mobile:vars.mobile,pattern:'detail',workplace:'view'})
							.then((param) => finish())
							.catch((error) => finish());
						};
						vars.records={};
									...
									return records.map((item) => {
										return {
											...
											extendedProps:{
												...
												url:kb.record.page.edit(vars.mobile,kb.config[PLUGIN_ID].app,item['$id'].value),
											}
										};
									});
								})(resp.reduce((result,current) => {
						...
						kb.field.load(kb.config[PLUGIN_ID].app,true).then((fieldInfos) => {
							...
							try
							{
								((setting) => {
									...
									if (current.setting.view.value==e.viewId) result=current.setting;
									return result;
								},null));
							}
							catch(error){
		...
		return new Promise((resolve,reject) => {
			((param) => {
				if (param)
				{
					((param) => {
						((button) => {
							button.parentNode.insertBefore(button.clone().on('click',(e) => {
								window.location.href=param.href;
							}),button.hide().nextElementSibling);
						})(kb.elm('.gaia-ui-actionmenu-cancel'));
						kintone.events.on([
