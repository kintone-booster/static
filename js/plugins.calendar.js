				...
				events:(info,success,failure) => {
					...
					.then((records) => {
						var recurse=(index,callback) => {
							...
							kb.event.call('kb.style.call',{record:records[index],mobile:vars.mobile,pattern:'detail',workplace:'view'})
							.then((param) => finish())
							.catch((error) => finish());
						};
						vars.records={};
						...
						kb.field.load(kb.config[PLUGIN_ID].app,true).then((fieldInfos) => {
							...
							try
							{
								((setting) => {
									...
									if (current.setting.view.value==e.viewId.toString()) result=current.setting;
									return result;
								},null));
							}
							catch(error)
