...
*/
"use strict";
window.BST_BREAK='__bst_break';
window.BST_CONTINUE='__bst__continue';
window.BST_SKIP='__bst__skip';
window.Booooooost=class{
	/* constructor */
	constructor(){
		/* setup properties */
		this.error=null;
		...
		this.file={
			clone:(files,silent=false) => {
				return new Promise((resolve,reject) => {
					var recurse=(index,callback) => {
						((file) => {
							bst.file.download(file,silent).then((blob) => {
								bst.file.upload(new File([blob],file.name,{type:file.contentType}),silent).then((resp) => {
			...
			download:(file,silent=false) => {
				return new Promise((resolve,reject) => {
					if (!silent) bst.loadStart();
					if (file.fileKey in bst.file.uploads)
					{
						if (!silent) bst.loadEnd();
						resolve(bst.file.uploads[file.fileKey]);
					}
					else
					{
				...
				return new Promise((resolve,reject) => {
					...
					.then((response) => {
						...
						response.json().then((json) => {
							...
							{
								case 200:
									bst.file.uploads[json.fileKey]=blob;
									resolve({
	...
	/* copy to clipboard */
	clipboard(value){
		var input=bst.create('input').attr('type','text').val(value);
		bst.elm('body').append(input);
		input.select();
	...
	elm(selectors){
		...
		*/
		...
		if (el) return el;
		if (selectors.startsWith('.kb-')) {
			el = document.querySelector(selectors.replace(/^\.kb-/, '.bst-'));
			if (el) return el;
		}
		if (selectors.startsWith('.bst-')) {
			el = document.querySelector(selectors.replace(/^\.bst-/, '.kb-'));
			if (el) return el;
		}
		return null;
	...
	elms(selectors){
		...
		*/
		...
		if (selectors.startsWith('.kb-')) {
			els = document.querySelectorAll(selectors.replace(/^\.kb-/, '.bst-'));
			if (els.length) return Array.from(els);
		}
		if (selectors.startsWith('.bst-')) {
			els = document.querySelectorAll(selectors.replace(/^\.bst-/, '.kb-'));
			if (els.length) return Array.from(els);
		}
		return [];
	...
	progressStart(max,message){
		this.loadEnd();
		this.popups.progress.show(max,message);
	}
	/* update progress */
	progressUpdate(){
	...
	ready(callback){
		if (!this.popups.alert)
		{
			...
			/* setup properties */
			this.apps=new BooooooostApps();
			this.error=new BooooooostError();
			this.event=new BooooooostEvent();
			this.oauth=new BooooooostOAuth();
			this.roleSet=new BooooooostRoleset();
			this.status=new BooooooostStatus();
			this.popups.datePicker=new BooooooostDatePicker();
			this.popups.colorPicker=new BooooooostColorPicker();
			this.popups.multiPicker=new BooooooostMultipicker();
			this.popups.progress=new BooooooostProgress();
			this.popups.loader=new BooooooostLoader();
			this.popups.alert=new BooooooostAlert();
			/* setup validation method */
			bst.elms('input,select,textarea').each((element,index) => element.initialize());
		}
		/* platform */
		switch (navigator.platform.replace(/^(iPad|iPhone|Linux|Mac|Win).*$/g,'$1'))
		{
			...
			case 'iPhone':
			case 'Mac':
				bst.elm('body').addClass('mac');
				break;
			case 'Linux':
				bst.elm('body').addClass('linux');
				break;
			case 'Win':
				bst.elm('body').addClass('win');
				break;
		}
...
window.BooooooostError=class{
	/* constructor */
	constructor(){
		this.config={
			reinstall:(name,url) => {
				var res='';
				switch (bst.operator.language)
				{
					case 'en':
						res='With the update of "'+name+'", it is necessary to reinstall the plugin.<br><a href="'+url+'" target="_blank">'+url+'</a><br>Please access this page to download the plugin and reload the plugin from the kintone System Administration page.';
						break;
					case 'ja':
						res=name+'のアップデートに伴い、プラグインの再インストールの必要があります。<br><a href="'+url+'" target="_blank">'+url+'</a><br>こちらのページにアクセスして、プラグインをダウンロードし、kintoneシステム管理ページからプラグインの再読み込みを行って下さい。';
						break;
					case 'zh':
						res='由于“'+name+'”的更新，需要重新安装插件。<br><a href="'+url+'" target="_blank">'+url+'</a><br>请访问此页面下载插件，并从 kintone 系统管理页面重新加载插件。';
						break;
					case 'zh-TW':
						res = '由於「'+name+'」的更新，需要重新安裝外掛程式。<br><a href="'+url+'" target="_blank">'+url+'</a><br>請訪問此頁面下載外掛程式，並從 kintone 系統管理頁面重新加載外掛程式。';
						break;
				}
				return res;
			},
			update:(name) => {
				...
				switch (bst.operator.language)
				{
					...
					case 'zh':
						...
						break;
					case 'zh-TW':
						res = '由於外掛程式「' + name + '」的更新，部分設定資訊已發生變更。<br>為了安全操作，請開啟外掛程式的設定頁面，不進行任何更改，然後直接點擊「保存」按鈕以更新設定。';
						break;
				}
				return res;
			}
...
window.BooooooostEvent=class{
	...
	on(events,handler){
		((Array.isArray(events))?events:events.split(',')).map((item) => item.trim()).each((type,index) => {
			if (type)
			{
				var eventType=type.replace(/^[^.]+\./, '');
				if (!(eventType in this.eventHandlers)) this.eventHandlers[eventType]=[];
	...
	off(events,handler){
		((Array.isArray(events))?events:events.split(',')).map((item) => item.trim()).each((type,index) => {
			if (type)
			{
				...
				{
					if (handler) this.eventHandlers[eventType]=this.eventHandlers[eventType].filter((item) => item!==handler);
					else this.eventHandlers[eventType]=[];
				}
			}
		});
		return this;
	...
	/* call event */
	call(type,param){
		var call=(index,eventType,param,callback) => {
			var handler=this.eventHandlers[eventType][index];
			var promise=(handler,param) => Promise.resolve().then(() => handler(param));
			promise(handler,param).then((resp) => {
				if (resp!== null && typeof(resp) === 'object' && resp.constructor === Object) param=resp;
				if (!param.error)
				{
					index++;
					if (index<this.eventHandlers[eventType].length) call(index,eventType,param,callback);
					else callback(param);
				}
				else callback(param);
			})
		...
		return new Promise((resolve,reject) => {
			...
			if (eventType in this.eventHandlers)
			{
				if (this.eventHandlers[eventType].length!=0) call(0,eventType,param,(param) => resolve(param));
				else resolve(param);
			}
			else resolve(param);
		});
...
window.BooooooostOAuth=class{
	/* constructor */
	constructor(){}
	/* authorize */
	authorize(clientId,clientSecret,scope,provider){
		return new Promise((resolve,reject) => {
			const popup=window.open('','popup',(() => {
				var res='width=500,height=600';
				var left=(window.screen.width / 2)-(500 / 2);
				var top=(window.screen.height / 2)-(600 / 2);
				return `width=500,height=600,top=${top},left=${left}`;
			})());
			const redirectUrl='https://api.kintone-booster.com/oauth/redirect';
			const authorizeUrl=(() => {
				var res='';
				...
				{
					case 'google':
						res='https://accounts.google.com/o/oauth2/v2/auth?client_id='+clientId+'&response_type=code&redirect_uri='+redirect+'&scope='+encodeURIComponent(scope)+'&state='+state+'&access_type=offline&prompt=consent';
						break;
					case 'microsoft':
						res='https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id='+clientId+'&response_type=code&redirect_uri='+redirect+'&scope='+encodeURIComponent(scope)+'&state='+state+'&prompt=consent';
						break;
				}
				return res;
			})();
			...
			const handler=(event) => {
				if (event.origin!==window.location.origin) reject(new Error('Origin mismatch'));
				if (event.data.code)
				{
					fetch(
						'https://api.kintone-booster.com/oauth/refresh',
						{
							method:'POST',
							headers:{
								'X-Requested-With':'XMLHttpRequest'
					...
					.then((response) => {
						response.json().then((json) => {
							if (response.ok)
							{
								...
								else reject(new Error('No refresh token found in response'));
							}
							else reject(new Error(bst.error.parse(json.error)));
						});
					})
					.catch((error) => {
						reject(new Error(bst.error.parse(error)));
					});
				}
				else
				{
	...
	callback(){
		...
		else
		{
			...
			if (error)
			{
				window.opener.postMessage({error:params.get('error')},window.location.origin);
				window.close();
				return true;
			}
		}
...
window.BooooooostRoleset=class{
	/* constructor */
	constructor(){
		/* setup properties */
		this.group=[];
...
window.BooooooostDialog=class{
	/* constructor */
	constructor(zIndex,disuseheader,disusefooter){
		/* setup properties */
		this.parts={
			button:bst.create('button').addClass('bst-dialog-button').css({
			...
			div:bst.create('div').css({
				boxSizing:'border-box',
				position:'relative',
				verticalAlign:'top'
			}),
			icon:bst.create('img').css({
				backgroundColor:'transparent',
				border:'none',
				boxSizing:'border-box',
				cursor:'pointer',
			...
			table:bst.create('table').css({
				border:'none',
				borderCollapse:'collapse',
				margin:'0',
				width:'100%'
			}),
			td:bst.create('td').css({
				border:'none',
		...
		this.cover=this.parts.div.clone().addClass('bst-dialog').css({
			alignItems:'center',
		...
		/* embed stylesheet */
		if (!bst.elm('.bst-dialog-style'))
		{
			bst.elm('head').append(
				bst.create('style')
				.addClass('bst-dialog-style')
				.attr('type','text/css')
				.text((() => {
					var res='';
					res+='.bst-dialog *{';
	...
	show(){
		this.container.css({
			backgroundColor:bst.themeColor().backcolor,
			color:bst.themeColor().forecolor
		});
...
window.BooooooostAlert=class extends BooooooostDialog{
	/* constructor */
	constructor(){
...
window.BooooooostCalendar=class{
	/* constructor */
	constructor(minimalise){
		/* setup properties */
		this.minimalise=minimalise;
		this.calendar=bst.create('table').css({
			borderCollapse:'collapse',
			margin:'0',
			width:'100%'
		});
		...
		((cell,weeks) => {
			...
			this.calendar.append(
				((body) => {
					(weeks.length*6).each((index) => {
						if (index%weeks.length==0) body.append(bst.create('tr'));
			...
			textAlign:'center',
			width:'calc(100% / 7)'
		}),bst.constants.weeks[bst.operator.language]);
	}
	/* show calendar */
...
window.BooooooostColorPicker=class extends BooooooostDialog{
	/* constructor */
	constructor(){
		...
		.attr('placeholder',bst.constants.dialog.color.prompt[bst.operator.language])
		.attr('type','text');
		...
		.on('click',(e) => {
			if (!this.input.val()) bst.alert(bst.constants.dialog.color.message.invalid[bst.operator.language]);
			else
			{
...
window.BooooooostDatePicker=class extends BooooooostDialog{
	/* constructor */
	constructor(){
		...
		this.calendar=new BooooooostCalendar();
...
window.BooooooostMultipicker=class extends BooooooostDialog{
	/* constructor */
	constructor(){
	...
	show(records,columnInfos,callback,selected){
		...
		if(records instanceof Array)
		{
			if (records.length!=0)
			{
				...
				.append(bst.create('thead').append(bst.create('tr')))
				.append(bst.create('tbody').append(bst.create('tr')));
			...
			else bst.alert(bst.constants.common.message.notfound[bst.operator.language]);
		}
		else bst.alert(bst.constants.common.message.invalid.parameter[bst.operator.language]);
	}
}
window.BooooooostLoader=class{
	/* constructor */
	constructor(){
		var span=bst.create('span');
		var keyframes={};
		var vendors=['-webkit-',''];
		/* initialize valiable */
		keyframes['0%']={
		...
		this.container=bst.create('p').css({
			bottom:'0',
			fontSize:'0.8em',
			height:'2em',
			left:'0',
	...
	/* show */
	show(){
		this.cover.css({display:'block',zIndex:bst.popups.alert.cover.style.zIndex-1});
	}
	/* hide */
	hide(){
...
window.BooooooostPopupform=class extends BooooooostDialog{
	/* constructor */
	constructor(innerElements,width,height,callbacks){
		super(999996,false,false);
		/* setup properties */
		if (bst.isNumeric(width)) this.container.css({width:width+'px'});
		else
		{
			if (width=='full') this.container.css({width:'100%'});
		}
		if (bst.isNumeric(height)) this.container.css({height:height+'px'});
		else
		{
...
window.BooooooostPopupwindow=class extends BooooooostDialog{
	/* constructor */
...
window.BooooooostProgress=class{
	/* constructor */
	constructor(){
		/* setup properties */
		this.counter=0;
		...
		this.monitor=bst.create('span').css({
			color:'#ffffff',
			display:'inline-block',
			lineHeight:'2em',
			verticalAlign:'top'
		});
		this.progress=bst.create('div').css({
...
HTMLDocument.prototype.off=function(type,handler){
	((Array.isArray(type))?type:type.split(',').map((item) => item.trim())).each((type,index) => {
		if (type)
		{
			...
			{
				if (this in bst.eventHandlers)
					if (type in bst.eventHandlers[this])
						bst.eventHandlers[this][type].each((handler,index) => this.removeEventListener(type,handler));
			}
		}
	});
	return this;
...
HTMLDocument.prototype.on=function(type,handler){
	((Array.isArray(type))?type:type.split(',').map((item) => item.trim())).each((type,index) => {
		if (type)
		{
			if (!(this in bst.eventHandlers)) bst.eventHandlers[this]={};
			if (!(type in bst.eventHandlers[this])) bst.eventHandlers[this][type]=[];
			bst.eventHandlers[this][type].push(handler);
...
HTMLElement.prototype.elm=function(selectors){
	...
	Temporarily retained until cached data is cleared.
	*/
	let el = this.querySelector(selectors);
	if (el) return el;
	if (selectors.startsWith('.kb-')) {
		el = this.querySelector(selectors.replace(/^\.kb-/, '.bst-'));
		if (el) return el;
	}
	if (selectors.startsWith('.bst-')) {
		el = this.querySelector(selectors.replace(/^\.bst-/, '.kb-'));
		if (el) return el;
	}
	return null;
...
HTMLElement.prototype.elms=function(selectors){
	...
	Temporarily retained until cached data is cleared.
	*/
	let els = this.querySelectorAll(selectors);
	if (els.length) return Array.from(els);
	if (selectors.startsWith('.kb-')) {
		els = this.querySelectorAll(selectors.replace(/^\.kb-/, '.bst-'));
		if (els.length) return Array.from(els);
	}
	if (selectors.startsWith('.bst-')) {
		els = this.querySelectorAll(selectors.replace(/^\.bst-/, '.kb-'));
		if (els.length) return Array.from(els);
	}
	return [];
...
HTMLElement.prototype.initialize=function(){
	this.alert=bst.create('div').css({
		display:'none',
		margin:'-0.5em 0px 0px 0px',
		transition:'none',
		zIndex:bst.popups.alert.cover.style.zIndex-4
	})
	.append(
		bst.create('img').css({
			display:'block',
	...
	var transition=(e) => {
		...
		if (code==13)
		{
			var elements=bst.elms('button[tabstop=tabstop],input,select,textarea').filter((element) => {
				var exists=0;
				if (element.visible())
				{
	...
	if (this.hasAttribute('required'))
	{
		...
		if (placeholder) placeholder=placeholder.replace(/^required /g,'').replace(/^\* /g,'')
	...
	return this.on('invalid',(e) => {
		/* validate required */
		if (e.currentTarget.validity.valueMissing) this.alert.elm('span').html(bst.constants.common.message.invalid.required[bst.operator.language]).parentNode.show();
		/* validate pattern */
		if (e.currentTarget.validity.patternMismatch) this.alert.elm('span').html(bst.constants.common.message.invalid.unmatch.pattern[bst.operator.language]).parentNode.show();
		/* validate type */
		if (e.currentTarget.validity.typeMismatch) this.alert.elm('span').html(bst.constants.common.message.invalid.unmatch.format[bst.operator.language]).parentNode.show();
...
HTMLElement.prototype.popup=function(width,height,buttons,callback){
	return new BooooooostPopupwindow(this,width,height,buttons,callback);
}
...
HTMLElement.prototype.siblings=function(selectors){
	var elements=(typeof selectors!=='undefined')?Array.from(this.parentNode.querySelectorAll(selectors)):bst.children(this.parentNode);
...
HTMLElement.prototype.spread=function(addcallback,delcallback,autoadd=true){
	...
	this.tr=bst.children(this.container);
...
HTMLSelectElement.prototype.filteroption=function(options){
	...
	if (((Array.isArray(options))?options:[]).length!=0)
	{
		this.elms('option').each((element,index) => {
			if (element.val() && !options.includes(element.val()))
			{
				((span) => {
				...
				})(bst.create('span').css({display:'none'}));
			}
		});
	}
	return this;
...
Array.prototype.each=function(handler){
	for (var i=0;i<this.length;i++)
	{
		...
		if (result===BST_BREAK) break;
		if (result===BST_SKIP) i++;
	}
}
...
Date.prototype.calc=function(pattern){
	...
	pattern.split(',').map((item) => item.trim()).each((pattern,index) => {
		...
		//first day of week
		if (pattern.match(/^first-of-week$/g)) day-=date.getDay();
		//add years
...
Date.prototype.format=function(pattern){
	...
	var week=bst.constants.weeks[bst.operator.language][this.getDay()];
	//iso 8601
	...
	//Others
	return pattern.replace(/Y/g,year).replace(/y/g,year.slice(-2)).replace(/m/g,month).replace(/d/g,day).replace(/H/g,hour).replace(/h/g,(hour%12).toString()).replace(/i/g,minute).replace(/s/g,second).replace(/w/g,week);
}
/*
FileList extention
*/
...
Number.prototype.comma=function(decimals){
	return (!isNaN(this))?((res) => {
		...
		return res.join('.');
	})((bst.isNumeric(decimals))?this.toFloor(parseInt(decimals)).split('.'):String(this).split('.')):'';
}
...
Number.prototype.toFloor=function(decimals){
	return ((res) => {
		...
		else res.push(''.lpad('0',decimals));
		if (Number(decimals)==0 && res.length!=1) res=[res[0]];
		return res.join('.');
	})(String(this).split('.'));
}
/*
...
if (!window.bst) window.bst=new Booooooost();
if (!window.kb) window.kb=window.bst;
/*
Message definition by language
*/
if (!window.bst.constants)
	bst.constants={
		common:{
			message:{
				invalid:{
...
bst.ready();
if (!window.bst_authorize_handler) window.bst_authorize_handler=(e) => {
	if (localStorage.getItem('bst-installed'))
	{
		((installed,subdomain) => {
				...
				((announced) => {
					if (announced!=new Date().format('Y-m-d'))
					{
						((announcetime) => {
							...
							{
								localStorage.setItem('bst-announced-'+kintone.app.getId(),new Date().format('Y-m-d'));
								localStorage.removeItem('bst-announcetime-'+kintone.app.getId());
															...
															(() => {
																...
																switch (bst.operator.language)
																{
																	...
																	case 'zh':
																		...
																		break;
																	case 'zh-TW':
																		res = 'Boost! 可以免費下載和使用，但若要無此訊息使用，您必須購買許可證。<br>您想繼續購買流程嗎？';
																		break;
																}
																return res;
															})(),
															() => window.open('https://booooooost.com/'+bst.operator.language.substring(0,2)+'/license.html?subdomain='+subdomain)
														);
													}
													break;
												default:
							...
							var res=null;
							if (localStorage.getItem('bst-announcetime-'+kintone.app.getId())) res=parseInt(localStorage.getItem('bst-announcetime-'+kintone.app.getId()));
							else
							{
								var hour=new Date().getHours();
								res=(hour<15)?(Math.floor(Math.random()*(15-hour))+hour):hour-1;
								localStorage.setItem('bst-announcetime-'+kintone.app.getId(),res.toString());
							}
							return res;
						})());
					}
				})((localStorage.getItem('bst-announced-'+kintone.app.getId()))?localStorage.getItem('bst-announced-'+kintone.app.getId()):new Date().calc('-1 day').format('Y-m-d'));
			}
		})(new Date(localStorage.getItem('bst-installed')),location.host.split('.')[0]);
	}
	else localStorage.setItem('bst-installed',new Date().format('Y-m-d'));
	return e;
};
kintone.events.off('app.record.index.show',window.bst_authorize_handler);
kintone.events.on('app.record.index.show',window.bst_authorize_handler);
/*
Temporarily retained until cached data is cleared.
*/
window.KintoneBooster=window.Booooooost;
window.KintoneBoosterApps=window.BooooooostApps;
window.KintoneBoosterError=window.BooooooostError;
window.KintoneBoosterEvent=window.BooooooostEvent;
window.KintoneBoosterOAuth=window.BooooooostOAuth;
window.KintoneBoosterRoleset=window.BooooooostRoleset;
window.KintoneBoosterStatus=window.BooooooostStatus;
window.KintoneBoosterDialog=window.BooooooostDialog;
window.KintoneBoosterAlert=window.BooooooostAlert;
window.KintoneBoosterCalendar=window.BooooooostCalendar;
window.KintoneBoosterColorPicker=window.BooooooostColorPicker;
window.KintoneBoosterDatePicker=window.BooooooostDatePicker;
window.KintoneBoosterMultipicker=window.BooooooostMultipicker;
window.KintoneBoosterLoader=window.BooooooostLoader;
window.KintoneBoosterPopupform=window.BooooooostPopupform;
window.KintoneBoosterPopupwindow=window.BooooooostPopupwindow;
window.KintoneBoosterProgress=window.BooooooostProgress;
window.KB_BREAK=window.BST_BREAK;
window.KB_CONTINUE=window.BST_CONTINUE;
window.KB_SKIP=window.BST_SKIP;
