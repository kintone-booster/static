			...
			clone:(files,silent=false) => {
				return new Promise((resolve,reject) => {
					var recurse=(index,callback) => {
						((file) => {
							kb.file.download(file,silent).then(function(blob){
								kb.file.upload(new File([blob],file.name,{type:file.contentType}),silent).then(function(resp){
	...
	constructor(){
		this.config={
			update:(name) => {
				...
				{
					case 'en':
						res='Due to an update of the plugin "'+name+'", some configuration information has changed.<br>Please open the plugin settings screen, and without making any changes, click the "Save" button to re-save.';
						break;
					case 'ja':
						res=name+'のアップデートに伴い、一部の設定情報に変更が生じております。<br>安全な運用のため、一度プラグインの設定画面を開き、特に変更を行わずに、そのまま「保存」ボタンをクリックして再保存をして下さい。';
						break;
					case 'zh':
						res='由于插件“'+name+'”的更新，部分配置信息已发生变化。<br>为了安全运行，请打开插件的设置页面，不做任何更改，直接点击“保存”按钮进行重新保存。';
						break;
				}
				return res;
			}
					...
					if (announced!=new Date().format('Y-m-d'))
					{
						((announcetime) => {
							if (announcetime<new Date().getHours())
							{
								...
								fetch(
									'https://api.kintone-booster.com/authorize?id='+subdomain,
									{
										method:'GET',
										headers:{
											'X-Requested-With':'XMLHttpRequest'
